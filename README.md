...
### **НАПОМЕНА!**	
Python скриптата за земање информации од Yahoo! finance e крстена main.py и е во директориумот на изворниот код. Истата не ја повикувам преку апликацијата бидеќи имав проблеми со интерпретерот, и не успеав да направам .exe датотека од скриптата. За да се спуштат најнови информации од Yahoo! finance ќе мора пред подигнувањето на CryptoWallet апликацијата да се повика оваа скрипта (во датотеката каде се наоѓа CryptoWallet.exe) и притоа треба да биде инсталиран [yfinance](https://pypi.org/project/yfinance/) модулот. По повикувањето на оваа скрипта се креираат потребните датотеки во директориумот каде се наоѓа самата скрипта. Доколку неможе да се изврши скриптата, датотеките од CryptoWallet/data копирајте ги во директориумот каде се наоѓа извршната верзија на апликацијата (пример CryptoWallet/CryptoWallet/bin/Release за Visual Studio) за да може да стартува апликацијата
...
BTC (Bitcoin), ETH (Ethereum), ADA (Cardano), DOGE (DogeCoin), SOL (Solana), BCH (BitcoinCash), VET (VeChain). Вредностите на секои од овие валути се земени преку Python скрипта користејќи го модулот [yfinance](https://pypi.org/project/yfinance/) (Ran Aroussi), кои во реално време зема информации од Yahoo! finance.
...
Доколку корисникот не е регистриран во системот, тој има можност да се регистрира со кликање на Create an Account. Откако корисникот ќе го направи тоа, му се појавува форма каде може да внесе корисничко име и лозинка, и доколку се е во ред информациите се зачувани во системот и сега корисникот може да се најави во системот.
